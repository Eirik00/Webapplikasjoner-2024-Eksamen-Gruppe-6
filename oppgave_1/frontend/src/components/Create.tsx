...
export default function Create() {
    ...
    const handleCourseFieldChange = (event: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
    ...
    const handleLessonFieldChange = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, index: number | null = null) => { // Copilot anbefaler å bruke event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement> for å håndtere endringer i input-feltene.
      const { name, value } = event.target;
      let text: { id: string; text: string }[]; // Copilot anbefaler å bruke { id: string; text: string }[] for å definere text.
      if (lessons[currentLesson]?.text?.length === 0) {
        text = [{ id: `${Math.floor(Math.random() * 1000 + 1)}`, text: "" }];
      }
      
      //Copilot anbefaler å bruke map for å oppdatere text.
      if (lessons[currentLesson]?.text?.length > 0) {
        text = lessons[currentLesson]?.text?.map((_text, i) => {
          if (i === index) {
            return { id: _text.id, text: name === "text" ? value : _text.text };
          }
          return _text;
        });
      }
      ...
      const updatedLessons = lessons.map((lesson, i) => {
        if (i === currentLesson) {
          return { ...lesson, [name]: value, text: text ?? [] };
        }
        return lesson;
      });
